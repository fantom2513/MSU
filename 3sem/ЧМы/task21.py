...
def R_K_2(func, x_0, y_0, func_cnt, n, len):
    ...
    if func_cnt == 1:
        ...
        for i in range(0, n):
            x_new = x_i + h
            grid[x_new] = y_i + (func(x_i, y_i) + func(x_new, y_i + h 
* func(x_i, y_i))) * h / 2
            y_i = grid[x_new]
            x_i = x_new
        return grid
...
def run(): # Основная программа
    cnt = int(input(
        "Введите 1, если хотите протестиров"
"ать решение дифференциального уравне"
"ния\nВведите 2,если хотите "
        "протестировать решение системы "
        "из двух дифференциальных уравнений\n"))
    global f, x_0, y_0, sol
    if cnt == 1:
        num = int(input("Введите номер уравнения,"
 "решение которого вы хотите получить:\n"
            "1: y' = (x-x^2)*y, y(0) = 0\n"
        ...
        print(" x\t\ty (R_K_2)\ty (R_K_4)\t y "
        "(Точное решение)\n")
        sols = list(sol(grid_2.keys()))
        for i in range(0, n):
            x = list(grid_2.keys())[i]
            print("%.6f\t%.6f\t%.6f\t\t%.6f\n" % (x, list(grid_2.value
s())[i],list(grid_4.values())[i], sols[i]))

        print()
        plt.subplot(121)
        x = np.linspace(x_0, x_0 + len, 5000)
        plt.plot(x, sol(x), c='red', lw=3, label='Точное решение')
        plt.plot(list(grid_2.keys()), list(grid_2.values()), '--', c='
blue', lw=3, label='y(R_K_2)')
        plt.grid()
        plt.xlabel('x')
        plt.ylabel('y')
        plt.legend()
        plt.subplot(122)
        plt.plot(x, sol(x), c='red', lw=3, label='Точное решeние')
        plt.plot(list(grid_4.keys()), list(grid_4.values()), '--', c='
blue', lw=3, label='y(R_K_4)')
        plt.grid()
        plt.xlabel('x')
        plt.ylabel('y')
        plt.legend()
        ...
        "уравнений,"
                " решение которой вы хотите "
                "получить:\n"
                "1: { (y_1)' = "
                "sin(2 * y_1^2) + y_2 + x, y_1(0) = 1,\n"
                " (y_2)' ="
                " x + y_1 - 2 * (y_2^2) + 1, y_2(0) = 0.5 }\n"
                "2: { (y_1)' = -y_2, y_1(0) = 1,\n"
                " (y_2)' = y_1, y_2(0) = 0 }\n"))
        if num == 1:
            f = [f1_1, f1_2]
...
vals_4_1[i], sols[i]))
            ...
            plt.subplot(221)
            x = np.linspace(x_0, x_0 + len, 5000)
            plt.plot(x, sol[0](x), c='red', lw=3, label='Точное 
решение')
            plt.plot(list(grid_2.keys()), vals_2_1, '--', c='blue', lw
=3, label='y_1 (R_K_2)')
            plt.grid()
            plt.xlabel('x')
            plt.ylabel('y')
            ...
            plt.plot(x, sol[0](x), c='red', lw=3, label='Точное 
решение')
            plt.plot(list(grid_4.keys()), vals_4_1, '--', c='blue', lw
=3, label='y_1 (R_K_4)')
            plt.grid()
            plt.xlabel('x')
            plt.ylabel('y')
            plt.legend(loc=0)
            print("\n x\t\ty_2 (R_K_2)\ty_2 (R_K_4)\t y (Точное 
решение)\n")
...
vals_4_2[i], sols[i]))
            ...
            plt.subplot(223)
            x = np.linspace(x_0, x_0 + len, 5000)
            plt.plot(x, sol[1](x), c='red', lw=3, label='Точное 
решение')
            plt.plot(list(grid_2.keys()), vals_2_2, '--', c='blue', lw
=3, label='y_2 (R_K_2)')
            plt.grid()
            plt.xlabel('x')
            plt.ylabel('y')
            ...
            plt.plot(x, sol[1](x), c='red', lw=3, label='Точное 
решение')
            plt.plot(list(grid_4.keys()), vals_4_2, '--', c='blue', lw
=3, label='y_2 (R_K_4)')
            plt.grid()
            plt.xlabel('x')
            plt.ylabel('y')
            ...
            for i in range(0, n):
                x = list(grid_2.keys())[i]
                print("%.6f\t%.6f\t%.6f\n" % (x, vals_2_1[i], vals_4_1
[i]))
            print()
            plt.subplot(221)
            plt.plot(list(grid_2.keys()), vals_2_1, c='blue', lw=3, la
bel='y_1 (R_K_2)')
            ...
            plt.legend(loc=0)
            plt.subplot(222)
            plt.plot(list(grid_4.keys()), vals_4_1, c='blue', lw=3, la
bel='y_1 (R_K_4)')
            ...
            for i in range(0, n):
                x = list(grid_2.keys())[i]
                print("%.6f\t%.6f\t%.6f\n" % (x, vals_2_2[i], vals_4_2
[i]))
            print()
            plt.subplot(223)
            plt.plot(list(grid_2.keys()), vals_2_2, c='blue', lw=3, la
bel='y_2 (R_K_2)')
            grid_2 = R_K_2(f, x_0, y_0, cnt, n // 2, len)
            vals_2_2 = list()
            for i in grid_2.values():
            ...
            plt.legend(loc=0)
            plt.subplot(224)
            plt.plot(list(grid_4.keys()), vals_4_2, c='blue', lw=3, la
bel='y_2 (R_K_4)')
            grid_4 = R_K_4(f, x_0, y_0, cnt, n // 2, len)
            vals_4_2 = list()
            for i in grid_4.values():
