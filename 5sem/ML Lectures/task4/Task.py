...
class MyOneHotEncoder(Preprocesser):
    ...
    def transform(self, X):
        ...
        for i in self.array:
            length.append(len(i))
        result = np.zeros((len(X), sum(length)))
        pos = 0
        summ = 0
        index = 0
        index2 = 0
        for i in X:
            for ind in X.index:
                a = X.loc[ind]
                result[pos %
                       len(result)][list(self.array[index]).index(a[i]) +
                                    summ] = 1
                pos += 1
            summ += length[index2]
            index2 += 1
    ...
    def transform(self, X, a=1e-5, b=1e-5):
        ...
        for i in X:
            for j in X.index:
                ...
                tmp_val = X.loc[j][i]
                new_arr2[ind + 2] = (new_arr[tmp_val][0] + a) / \
                    (new_arr[tmp_val][1] + b)
...
class FoldCounters:
    ...
    def fit(self, X, Y, seed=1):
        ...
        for i1, j1 in group_k_fold(
                len(array), n_splits=self.n_folds, seed=seed):
            for i in range(0, array[i1].shape[1]):
                for j in range(0, len(array[i1])):
                    ...
                    for k in range(0, len(array[j1])):
                        ...
                        summ2 += np.array(Y[j1])[k] * \
                            int(array[j1][k][i] == array[i1][j][i])
